...
export default class FactoryNode extends Component {
  ...
  // Updates state when new data is sent
  componentDidMount = () => {
  ...
  handleRandomNumbersSubmit = event => {
    ...
    axios
      .post(`/api/updateNumbers`, { id, numbers })
      .then(() => {
...
// 2) Add a click field covering rest of page so that when clicked on it will close FactoryNode actions box so it doesn't hover.
...
// 5) Add error catching and message displaying for errors